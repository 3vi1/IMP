...
{
    SliderDialog dialog;
    dialog.setWindowTitle("Set Line Width");

    float r;
    if(m_theme->attributeExists("lineWidth"))
    {
        r = m_theme->getAttribute("lineWidth").toFloat();
    }
    else
    {
        r = 1.0f;
    ...
    {
        m_theme->setAttribute("lineWidth", LINES, "", "width", qreal(dialog.value()));
    }
}

void MainWindow::on_actionLineOpacity_triggered()
{
    SliderDialog dialog;
    dialog.setWindowTitle("Set Line Opacity");
    dialog.setUnit("");

    qreal opacity;
    if(m_theme->attributeExists("lineOpacity"))
    {
        opacity = m_theme->getAttribute("lineOpacity").toDouble();
    }
    else
    {
        opacity = 1.0;
    }
    dialog.setValue(opacity*100);

    if(dialog.exec() == QDialog::Accepted)
    {
        m_theme->setAttribute("lineOpacity", LINES, "", "opacity", qreal(dialog.value()/100.0f));
    }
}
...
void MainWindow::on_actionLineColor_triggered()
{
    ...
    if(c.isValid())
        m_theme->setAttribute("lineColor", LINES, "", "color", c);
}

void MainWindow::opacityChange(QString obj, ThemeObject objType)
{
    SliderDialog dialog;
    dialog.setWindowTitle("Set " + obj + " Opacity");
    dialog.setUnit("");

    qreal opacity;
    if(m_theme->attributeExists(obj.toLower() + "Opacity"))
    {
        opacity = m_theme->getAttribute(obj.toLower() + "Opacity").toDouble();
    }
    else
    {
        opacity = 1.0;
    ...
    if(dialog.exec() == QDialog::Accepted)
    {
        m_theme->setAttribute(obj.toLower() + "Opacity",
                              objType,
                              "",
                              "opacity",
                              qreal(dialog.value()/100.0f));
    }
}

void MainWindow::on_actionPilotOpacity_triggered()
{
    opacityChange("Pilot", PILOT);
}

void MainWindow::scaleChange(QString obj, ThemeObject objType)
{
    SliderDialog dialog;
    dialog.setWindowTitle("Set " + obj + " Scale");
    dialog.setMin(1);
    dialog.setMax(1000);
    dialog.setDivisor(100);
    dialog.setUnit("x");

    qreal s;
    if(m_theme->attributeExists(obj.toLower() + "Scale"))
    {
        s = m_theme->getAttribute(obj.toLower() + "Scale").toDouble();
    }
    else
    {
        s = 1.0;
    }
    dialog.setValue(s*100);

    if(dialog.exec() == QDialog::Accepted)
    {
        qDebug() << "Scale = " << qreal(dialog.value()/100.0f);
        m_theme->setAttribute(obj.toLower() + "Scale",
                              objType,
                              "",
                              "scale",
                              qreal(dialog.value()/100.0f));
...
void MainWindow::on_actionPilotScale_triggered()
{
    scaleChange("Pilot", PILOT);
}

void MainWindow::on_actionPilotGraphic_triggered()
{
    QString fileName = QFileDialog::getOpenFileName(
                this, tr("Graphic for Pilot Indicator"),
                QString(),
                tr("Image Files (*.svg)") );

    if(fileName.length() > 0)
    {
        m_theme->setAttribute("pilotGraphic", PILOT, "", "graphic", fileName);
    }
}

void MainWindow::on_actionFindGraphic_triggered()
{
    QString fileName = QFileDialog::getOpenFileName(
                this, tr("Graphic for Find Indicator"),
                QString(),
                tr("Image Files (*.svg)") );

    if(fileName.length() > 0)
    {
        m_theme->setAttribute("findGraphic", FIND, "", "graphic", fileName);
    }
}

void MainWindow::on_actionFindOpacity_triggered()
{
    opacityChange("Find", FIND);
}

void MainWindow::on_actionFindScale_triggered()
{
    scaleChange("Find", FIND);
}

void MainWindow::customizeShape(QString obj, ThemeObject objType)
{
    CustomizationDialog dialog;
    dialog.setWindowTitle("Set " + obj + " Offsets");

    qreal d;
    if(m_theme->attributeExists(obj.toLower() + "XOffset"))
    {
        d = m_theme->getAttribute(obj.toLower() + "XOffset").toDouble() * -1;
    }
    else
    {
        d = 0.0;
    ...
    dialog.setXValue(d);
    if(m_theme->attributeExists(obj.toLower() + "YOffset"))
    {
        d = m_theme->getAttribute(obj.toLower() + "YOffset").toDouble() * -1;
    }
    else
    {
        d = 0.0;
    ...
    dialog.setYValue(d);
    if(m_theme->attributeExists(obj.toLower() + "Z"))
    {
        d = m_theme->getAttribute(obj.toLower() + "Z").toDouble();
    }
    else
    {
        d = 0.0;
    ...
    if(dialog.exec() == QDialog::Accepted)
    {
        m_theme->setAttribute(obj.toLower() + "XOffset",
                              objType,
                              "",
                              "xOffset",
                              qreal(dialog.xValue()*-1));
        m_theme->setAttribute(obj.toLower() + "YOffset",
                              objType,
                              "",
                              "yOffset",
                              qreal(dialog.yValue()*-1));
        m_theme->setAttribute(obj.toLower() + "Z",
                              objType,
                              "",
                              "z",
                              qreal(dialog.zValue()));
...
void MainWindow::on_actionFindX_Y_Offsets_triggered()
{
    customizeShape("Find", FIND);
}

void MainWindow::on_actionPilotX_Y_Offsets_triggered()
{
...
void MainWindow::on_actionFontSystemName_triggered()
{
    ...
    QFont font = QFontDialog::getFont(
                &ok, ui->mapView->getNameFont(), this, "Select System Name Font");
...
void MainWindow::on_actionFontTimer_triggered()
{
    bool ok = false;
    QFont font = QFontDialog::getFont(
                &ok, ui->mapView->getTimeFont(), this, "Select System Timer Font");
...
void MainWindow::on_actionBackgroundImage_triggered()
{
    ...
    if(fileName.length() > 0)
    {
        m_theme->setAttribute("backgroundGraphic", SYSTEM, "", "background", fileName);
    }
